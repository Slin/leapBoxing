...
namespace RN
{
	...
	inline Vector3 Matrix::operator* (const Vector3 &other) const
	{
		Vector3 result;
	...
	inline Vector4 Matrix::operator* (const Vector4 &other) const
	{
		Vector4 result;
	...
	inline Matrix Matrix::WithIdentity()
	{
		Matrix mat;
	...
	inline Matrix Matrix::WithTranslation(const Vector3 &translation)
	{
		Matrix mat;
	...
	inline Matrix Matrix::WithTranslation(const Vector4 &translation)
	{
		Matrix mat;
	...
	inline Matrix Matrix::WithScaling(const Vector3 &scaling)
	{
		Matrix mat;
	...
	inline Matrix Matrix::WithScaling(const Vector4 &scaling)
	{
		Matrix mat;
	...
	inline Matrix Matrix::WithRotation(const Vector3 &rotation)
	{
		Quaternion quat(rotation);
		return quat.GetRotationMatrix();
	}
	
	inline Matrix Matrix::WithRotation(const Vector4 &rotation)
	{
		Quaternion quat(rotation);
		return quat.GetRotationMatrix();
	}
	
	inline Matrix Matrix::WithRotation(const Quaternion &rotation)
	{
		Quaternion quat(rotation);
		return quat.GetRotationMatrix();
	}
	...
	inline Matrix Matrix::WithProjectionOrthogonal(float left, float right, float bottom, float top, float clipnear, float clipfar)
	{
		Matrix mat;
	...
	inline Matrix Matrix::WithProjectionPerspective(float arc, float aspect, float clipnear, float clipfar)
	{
		Matrix mat;
	...
	inline Matrix Matrix::WithInverseProjectionPerspective(float arc, float aspect, float clipnear, float clipfar)
	{
		Matrix mat;
	...
	inline float Matrix::GetSubmatrixDeterminant(const int k) const
	{
		int i = k%4; // i <=> j
		int j = k/4;
		int i1 = (i == 0);
	...
	inline Vector3 Matrix::GetEulerAngle() const
	{
		Vector3 result;
	...
	inline Vector4 Matrix::GetAxisAngle() const
	{
	...
	inline void Matrix::Translate(const Vector3 &translation)
	{
#if RN_SIMD
	...
	inline void Matrix::Translate(const Vector4 &translation)
	{
#if RN_SIMD
		SIMD::VecFloat result = SIMD::Mul(vec[0], SIMD::Set(translation.x));
		result = SIMD::Add(result, SIMD::Mul(vec[1], SIMD::Set(translation.y)));
	...
	inline void Matrix::Scale(const Vector3 &scaling)
	{
		*this *= Matrix::WithScaling(scaling);
	}
	
	inline void Matrix::Scale(const Vector4 &scaling)
	{
		*this *= Matrix::WithScaling(scaling);
	}
	
	inline void Matrix::Rotate(const Vector3 &rotation)
	{
		*this *= Matrix::WithRotation(rotation);
	}
	
	inline void Matrix::Rotate(const Vector4 &rotation)
	{
		*this *= Matrix::WithRotation(rotation);
	}
	
	inline void Matrix::Rotate(const Quaternion &rotation)
	{
		*this *= Matrix::WithRotation(rotation);
	}
	...
	inline bool Matrix::IsEqual(const Matrix &other, float epsilon) const
	{
