...
{	
	...
	inline Quaternion::Quaternion(const Vector4 &axis)
	{
	...
	inline bool Quaternion::operator== (const Quaternion &other) const
	{
		...
		if(fabs(y - other.y) > std::numeric_limits<float>::epsilon())
			return false;
		
		if(fabs(z - other.z) > std::numeric_limits<float>::epsilon())
			return false;
		
		if(fabs(w - other.w) > std::numeric_limits<float>::epsilon())
			return false;
	...
	inline bool Quaternion::operator!= (const Quaternion &other) const
	{
		if(fabs(x - other.x) <= std::numeric_limits<float>::epsilon() && fabs(y - other.y) <= std::numeric_limits<float>::epsilon() && fabs(z - other.z) <= std::numeric_limits<float>::epsilon() && fabs(w - other.w) <= std::numeric_limits<float>::epsilon())
			return false;
		
		return true;
	}
	
	inline Quaternion Quaternion::WithIdentity()
	{
		return Quaternion();
	}
	
	inline Quaternion Quaternion::WithEulerAngle(const Vector3 &euler)
	{
		Quaternion temp;
		
		const float Pi_360(M_PI / 360.0f);
	...
	inline Quaternion Quaternion::WithAxisAngle(const Vector4 &axis)
	{
		Quaternion temp;
	...
	inline Quaternion Quaternion::WithLerpSpherical(const Quaternion &start, const Quaternion &end, float factor)
	{
		...
		else
		{
			...
			scale = Math::Sin(M_PI * (0.5f - factor));
			inverseScale = Math::Sin(M_PI * factor);
		}
		
		return (quat1 * scale) + (quat2 * inverseScale);
	}
	
	inline Quaternion Quaternion::WithLerpLinear(const Quaternion &start, const Quaternion &end, float factor)
	{
		float inverseFactor = 1.0f - factor;
		return (end * factor) + (start * inverseFactor);
	}
	
	inline Quaternion Quaternion::WithLookAt(const Vector3 &tdir, const Vector3 &tup, bool forceup)
	{
		Quaternion temp;
	...
	inline Quaternion Quaternion::GetLerpSpherical(const Quaternion &other, float factor) const
	{
		return WithLerpSpherical(*this, other, factor);
	}
	
	inline Quaternion Quaternion::GetLerpLinear(const Quaternion &other, float factor) const
	{
		return WithLerpLinear(*this, other, factor);
	}
	
	inline Vector3 Quaternion::GetRotatedVector(const Vector3 &vector) const
	{
	...
	inline Vector4 Quaternion::GetRotatedVector(const Vector4 &vector) const
	{
		Quaternion vectorquat(vector.x, vector.y, vector.z, 0.0f);
		Quaternion resultquat = (*this) * vectorquat * GetConjugated();
	...
	inline Matrix Quaternion::GetRotationMatrix() const
	{
		Matrix result;
	...
	inline Vector4 Quaternion::GetAxisAngle() const
	{
		Vector4 res;
	...
	inline float Quaternion::GetDotProduct(const Quaternion &other) const
	{
		return x * other.x + y * other.y + z * other.z + w * other.w;
	}
	
	inline bool Quaternion::IsEqual(const Quaternion &other, float epsilon) const
	{
