...
namespace RN
{
	...
	inline bool Vector2::operator!= (const Vector2 &other) const
	{
		if(fabs(x - other.x) <= std::numeric_limits<float>::epsilon() && fabs(y - other.y) <= std::numeric_limits<float>::epsilon())
			return false;
	...
	{
		if(x*x+y*y > std::numeric_limits<float>::epsilon())
		{
	...
	inline bool Vector3::operator!= (const Vector3 &other) const
	{
		if(fabs(x - other.x) <= std::numeric_limits<float>::epsilon() && fabs(y - other.y) <= std::numeric_limits<float>::epsilon() && fabs(z - other.z) <= std::numeric_limits<float>::epsilon())
			return false;
	...
	{
		if(x*x+y*y+z*z > std::numeric_limits<float>::epsilon())
		{
...
#if RN_SIMD
	inline Vector4::Vector4(const SIMD::VecFloat &other) :
		simd(other)
	{}
	
	inline Vector4 &Vector4::operator= (const SIMD::VecFloat &other)
	{
		simd = other;
		return *this;
	}
	...
	inline Vector4 &Vector4::Normalize(const float n)
	{
		...
		if(x*x+y*y+z*z+w*w > std::numeric_limits<float>::epsilon())
		{
